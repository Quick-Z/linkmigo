...
const INSTAGRAM_AVATAR_HEADERS = {
...
export async function GET(request) {
  try {
    ...
    const sourceUrl = normalizeInstagramAvatarUrl(rawUrl);
    const response = await fetchWithTimeout(
      sourceUrl,
      {
        cache: "no-store",
        headers: INSTAGRAM_AVATAR_HEADERS,
      },
      getSocialDownloaderSettings().httpTimeoutMs,
    );

    if (response.status === 404) {
      throw new AppError(ErrorCode.NO_MEDIA_FOUND, "头像图片不存在。", 404);
    }

    if (response.status >= 400) {
      throw new AppError(ErrorCode.UPSTREAM_BLOCKED, `头像图片请求失败（${response.status}）。`, 502);
    }
    ...
    if (!contentType.toLowerCase().startsWith("image/")) {
      throw new AppError(ErrorCode.NO_MEDIA_FOUND, "头像地址没有返回图片。", 404);
    }

    if (Number.isFinite(contentLength) && contentLength > AVATAR_MAX_BYTES) {
      throw new AppError(ErrorCode.NO_MEDIA_FOUND, "头像图片过大。", 413);
    }
...
function normalizeInstagramAvatarUrl(value) {
  const rawUrl = String(value || "").trim();

  if (!rawUrl || rawUrl.length > 8192) {
    throw new AppError(ErrorCode.UNSUPPORTED_URL, "头像地址无效。", 400);
  }
  ...
  try {
    ...
    throw new AppError(ErrorCode.UNSUPPORTED_URL, "头像地址格式无效。", 400, getErrorDetail(error));
  }

  if (!["http:", "https:"].includes(parsed.protocol) || !isAllowedInstagramImageHost(parsed.hostname)) {
    throw new AppError(ErrorCode.UNSUPPORTED_URL, "仅支持代理 Instagram 头像图片。", 400);
  }
